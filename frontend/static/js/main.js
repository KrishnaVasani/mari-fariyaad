...
function switchLanguage(langKey) {
  ...
  initLanguage();
  // Notify other components (e.g. the virtual keyboard) so they can react
  // to the new language immediately, without a page reload.
  document.dispatchEvent(new CustomEvent("gvp:languagechange", { detail: { lang: langKey } }));
}

/**
 * Small helper so JS-generated strings (validation errors, success/empty
 * messages) also come from the centralized translation files instead of
 * being hard-coded per component. Falls back to the key itself if the
 * translation hasn't loaded yet or the key is missing.
 */
function t(key) {
...
async function handleRegisterSubmit(e) {
  ...
  if (payload.password !== payload.confirmPassword) {
    showAlert(alertBox, t("msg_password_mismatch"));
    return;
  }
...
async function handleComplaintSubmission(e) {
  ...
  try {
    ...
    if (successModal) {
      ...
      alert(`${t("msg_complaint_submitted")} ${complaint.ticketId}`);
...
async function renderUserDashboard() {
  ...
  try {
    ...
    if (tbody) {
      if (list.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="text-center py-4 text-muted">${t("msg_no_complaints")} <a href="/complaint.html">Register a Complaint</a></td></tr>`;
        return;
      }
...
async function renderAdminDashboard() {
  ...
  try {
    ...
    renderAdminCategoriesBreakdown(adminComplaintsCache);
    renderAdminReports(adminComplaintsCache);
...
/* Groups complaints by a key (category or building) and counts each status bucket. */
function groupComplaintsBy(list, keyFn) {
  const groups = {};
  list.forEach((c) => {
    const key = keyFn(c) || "Uncategorized";
    if (!groups[key]) groups[key] = { total: 0, Pending: 0, ProgressGroup: 0, Resolved: 0, Rejected: 0 };
    const g = groups[key];
    g.total++;
    if (c.status === "Pending") g.Pending++;
    else if (c.status === "Assigned" || c.status === "In Progress") g.ProgressGroup++;
    else if (c.status === "Resolved") g.Resolved++;
    else if (c.status === "Rejected") g.Rejected++;
  });
  return groups;
}

function renderAdminCategoriesBreakdown(list) {
  const tbody = document.getElementById("adminCategoriesTbody");
  if (!tbody) return;
  const groups = groupComplaintsBy(list, (c) => c.categoryName || c.category);
  const keys = Object.keys(groups).sort((a, b) => groups[b].total - groups[a].total);

  if (keys.length === 0) {
    tbody.innerHTML = `<tr><td colspan="6" class="text-center py-4 text-muted">No complaints filed yet.</td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = keys.map((key) => {
    const g = groups[key];
    return `
      <tr style="cursor:pointer" onclick="filterAdminDashboardAndSwitchTab('${escapeHtml(key)}')">
        <td><strong>${escapeHtml(key)}</strong></td>
        <td>${g.total}</td>
        <td>${g.Pending}</td>
        <td>${g.ProgressGroup}</td>
        <td>${g.Resolved}</td>
        <td>${g.Rejected}</td>
      </tr>`;
  }).join("");
...
function renderAdminReports(list) {
  const summaryEl = document.getElementById("adminReportStatusSummary");
  if (summaryEl) {
    const total = list.length;
    const resolved = list.filter((c) => c.status === "Resolved").length;
    const rejected = list.filter((c) => c.status === "Rejected").length;
    const openCount = total - resolved - rejected;
    const resolutionRate = total > 0 ? Math.round((resolved / total) * 100) : 0;

    const cards = [
      { label: "Total Filed", value: total, cls: "border-primary text-dark" },
      { label: "Still Open", value: openCount, cls: "border-warning text-warning" },
      { label: "Resolved", value: resolved, cls: "border-success text-success" },
      { label: "Resolution Rate", value: resolutionRate + "%", cls: "border-info text-info" },
    ];
    summaryEl.innerHTML = cards.map((c) => `
      <div class="col-md-3">
        <div class="gvp-card p-3 border-start border-4 ${c.cls}">
  ...
  const tbody = document.getElementById("adminReportBuildingTbody");
  if (!tbody) return;
  const groups = groupComplaintsBy(list, (c) => c.building);
  const keys = Object.keys(groups).sort((a, b) => groups[b].total - groups[a].total);
...
/* Clicking a category row in the Categories tab jumps back to the Dashboard
   tab with the complaint table filtered to that category. */
function filterAdminDashboardAndSwitchTab(categoryKey) {
  switchAdminTab("dashboard");
  const filterInput = document.getElementById("adminFilterInput");
  const filtered = adminComplaintsCache.filter(
    (c) => (c.categoryName || c.category) === categoryKey
  );
  renderAdminComplaintsTable(filtered);
  if (filterInput) filterInput.value = categoryKey;
}

function switchAdminTab(tabName) {
  ["dashboard", "categories", "reports"].forEach((name) => {
    const panel = document.getElementById("adminTab" + name.charAt(0).toUpperCase() + name.slice(1));
    if (panel) panel.style.display = name === tabName ? "" : "none";
  });
  document.querySelectorAll("[data-admin-tab]").forEach((link) => {
    link.classList.toggle("active", link.dataset.adminTab === tabName);
  });
}
